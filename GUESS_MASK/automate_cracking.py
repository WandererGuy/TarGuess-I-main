...
def replace_format(max_mask_generate: int, 
                   info_dict: dict, 
                   train_result_refined_path: str
                   ) -> tuple[list[str], list[str]]:
    '''
    create mask with target info replace in it 
    first, create format dict
    second, read refined train result and replace mask with format dict
    return raw mask list (original mask list) and new mask list (new mask list)
    '''
        ...
        for line in tqdm(lines, total = len(lines)):
            ...
            for i in range(len(raw)):
                if raw[i] in format_dict.keys():
                    if format_dict[raw[i]] == '': 
                        # skip mask with empty fill even 1 target info component in mask 
...
def generate_mask_file(mask_file_path, file_path):
    '''
    refined new mask list into a better mask format 
    then collect those masks
    '''
    mask_ls = []
...
def main() -> None:
    '''
    given target info 
    generate mask file (in format can be pass into hashcat)
    and generate mask with probability (for later pcfg wordlist creation)
    '''
    parser = argparse.ArgumentParser(description="parse input data")
    parser.add_argument('--mask_file_path', type=str, help='path to output target info file')
    parser.add_argument('--target_info_file', type=str, help='path to target info file')
    parser.add_argument('--max_mask_generate', type=str, help='max mask to generate') 
    # max mask can generate if full all information 
    # less mask if less information 
    parser.add_argument('--train_result_refined_path', type=str, help='path to train_result_refined_path')
    parser.add_argument('--mask_prob_path', type=str, help='path to output target mask with probability')
    ...
    with open(f, 'w') as file:
        raw_lst, new_lst = replace_format(max_mask_generate, 
                                          info_dict, 
    ...
    mask_ls = generate_mask_file(mask_file_path, 
                                 f)
    with open (train_result_refined_path, 'r') as file:
        lines = file.readlines()
        for line in lines:
